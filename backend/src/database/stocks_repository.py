...
def get_ticker_history(ticker_name: str, period: str, interval: str) -> dict:
    dat = yf.Ticker(ticker_name)
    return dat.history(period=period, interval=interval).reset_index()

def get_ticker_history_from_start(ticker_name: str, start: str, interval: str) -> dict:
    dat = yf.Ticker(ticker_name)
    return dat.history(start=start, interval=interval).reset_index()

def get_tickers_history(ticker_names: list[str], period: str) -> dict:
...
def get_ticker_analyst_price_targets(ticker_name: str) -> dict:
    dat = yf.Ticker(ticker_name)
...
def get_tickers(ticker_names: list[str]) -> dict:
