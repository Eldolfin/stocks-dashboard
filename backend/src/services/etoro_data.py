...
def extract_closed_position(
    etoro_statement_file: Path, time_unit: str = "m"
) -> dict[str, list[str]]:
...
def extract_portfolio_evolution(etoro_statement_file: Path) -> pd.DataFrame:
    excel = pd.read_excel(etoro_statement_file, sheet_name=None)
    ...
    for tick in still_open["Details"].unique():
        ...
        _ticker_positions = _ticker_positions.reindex(
            pd.date_range(
                _ticker_positions.index.min(), pd.Timestamp.today()
            ),
            fill_value=0,
        )
    ...
    for (_ticker_name, _ticker) in shares_per_ticker.items():
        if _ticker_name in yahoo_data:
            ...
            _ticker.index = pd.to_datetime(
                _ticker.index
            ).tz_localize(None)
            _yahoo_data.index = pd.to_datetime(
                _yahoo_data.index
            ).tz_localize(None)
            combined = _ticker.join(_yahoo_data, how="left")
            combined["net_value"] = (
                combined["Close"] * combined["shares_sum"]
            )
    ...
    all_profits["Closed Positions"] = closed.rename(
