...
from .. import models
from ..database import stocks_repository
...
def get_ticker(query: models.TickerQuery):
    n_points = 20
    if query.period == "max":
        history = stocks_repository.get_ticker_history(query.ticker_name, "max", "3mo")
    ...
    if query.period == "max":
        history = stocks_repository.get_ticker_history(query.ticker_name, "max", query.interval)
    else:
        history = stocks_repository.get_ticker_history_from_start(query.ticker_name, start, query.interval)
    if history.empty:
        return None
...
def analyze_etoro_excel(form: models.EtoroForm, user_email: str):
...
def list_etoro_reports(user_email: str):
...
def analyze_etoro_excel_by_name(query: models.EtoroAnalysisByNameQuery, user_email: str):
    user_etoro_folder = Path(current_app.config["UPLOAD_FOLDER"]) / user_email
