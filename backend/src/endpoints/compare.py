...
from ..services import etoro_data
from .. import models
...
def compare_to_index():
    ...
    if not filename or not index_ticker:
        return jsonify({"error": "Missing filename or index_ticker"}), 400
    ...
    try:
        ...
        if not deposits:
            return jsonify({"error": "No deposit data found"}), 400
    except Exception as e:
        return jsonify({"error": f"Failed to parse eToro report: {str(e)}"}), 500

    # Fetch index historical prices
    try:
        # yfinance end is exclusive; extend by one day to cover the last date
        index_data = yf.download(index_ticker, start=dates[0], end=pd.to_datetime(dates[-1]) + pd.Timedelta(days=1))
            ...
            if isinstance(cols, pd.MultiIndex):
                ...
                try:
                    ...
                    try:
                        ...
                        return jsonify({"error": "No close price column in index data (multiindex)"}), 404
                # If multiple columns, pick the first
                if isinstance(sub, pd.DataFrame):
                    index_prices = sub.iloc[:, 0]
                else:
                    index_prices = sub
            else:
                price_col = None
                ...
                if price_col is None:
                    return jsonify({"error": "No close price column in index data"}), 404
                index_prices = index_data[price_col]
        else:
            return jsonify({"error": "Unexpected index data format"}), 500
        ...
        index_prices = index_prices.reindex(dates).ffill().bfill()
    except Exception as e:
    ...
    try:
        ...
        index_values = [float(u) * float(index_prices.loc[date]) if date in index_prices.index else 0.0 for u, date in zip(units_history, dates)]
    except Exception as e:
        return jsonify({"error": f"Simulation error: {str(e)}"}), 500

    return jsonify({"dates": dates, "index_values": index_values})
